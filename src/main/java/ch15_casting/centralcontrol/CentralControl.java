...
// Main에서 printer1을 centralControl에 추가, on() / off()를 실행한 후에 print() 메서드도 centralControl을 통해 실행하시오.
    ...
    public void addDevice(Power A) {
        ...
        deviceArray[emptyIndex] = A;
        System.out.println(A.getClass().getSimpleName() + " 장치가 연결되었습니다.");
    }
    ...
    public void powerOn() {
        for (int i = 0; i < deviceArray.length; i++) {
            if (deviceArray[i] == null) {
                System.out.println("슬롯 [" + (i + 1) + "]번: 장치 없음, 전원 켜지지 않음.");
                continue;
            }
            deviceArray[i].on();
        }
    ...
    // 각 장치에 특화된 메서드 실행 (장치 배열 전체에 대해)
    public void performSpecificMethods() {
        for (int i = 0; i < deviceArray.length; i++) {
            Power device = deviceArray[i];
            if (device == null) {
                System.out.println("슬롯 [" + (i + 1) + "]번 : 장치 없음");
                continue;
            }
            ...
            if (device instanceof AirConditioner airConditioner) {
                ...
                computer.computer();
            } else if (device instanceof LED led) {
                led.changeColor();
            } else if (device instanceof Mouse mouse) {
                mouse.leftClick();
            } else if (device instanceof Speaker speaker) {
                speaker.changeEqual();
            } else if (device instanceof Printer printer) {
                printer.print(); // 가정: Printer 클래스에 print() 메서드가 있다고 가정
            } else {
                System.out.println("알 수 없는 장치입니다.");
            }
        }
    }
