...
public class TeacherMain {
    public static void main(String[] args) {
        Teacher teacher1 = new Teacher("안근수","코리아타운");
        Teacher teacher2 = new Teacher("안근수","코리아타운");

        boolean result1 = teacher1.equals(teacher2);
        System.out.println(result1);        //true
        //이상의 코드에서 중요한 점은 재정의 전과 후가 결과값이 다르다는 겁니다.
        // Teacher에서 .equals()를 재정의하기 전에는 false가 떴었습니다. 그때의 경우는 teacher1과 teacher2의 주소지까지 확인하여 같은지 아닌지의 여부를 판단했다면,
        // 재정의 이후에는 field에 입력된 데이터가 동일한지만을 체크하기 때문입니다.

        Class tClass = teacher1.getClass(); // 지난 시간에 하긴 했습니다.
        System.out.println(tClass); // 결과값: class ch16_object.teacher.Teacher
        System.out.println(tClass.getSimpleName()); // 결과값: Teacher
        System.out.println(teacher1.getClass().getSimpleName()); // 결과값 : Teacher
        //여러분은 어느 수준까지 그냥 작성할지, 변수에 대입할지를 조직에 따라, 여러분 성향에 따라서 결정하셔야 합니다.
