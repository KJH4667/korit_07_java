...
/*
        super 키워드
    1. super();     -> 얘는 부모 생성자를
    2. super.메서드명();    -> 얘는 부모 클래스의 메서드르 호출한다는 의미


지금 작성하는 부분의 경우 github에 완전판이 올라가기 때문에 왜 오류가 발생하는지 어떻게 검증하는지 어려움을
겪을수 있습니다.

부모-자식의 상속관계가 성립되어 있을때,
부모 클래스에서 기본 생성자 없이 매개변수 생성자만 있다면 자식 클래스에서 완벽한 기본 생성자를 만나는 것이 불가능합니다.

자식 클래스의 객체 생성 시 필수적으로 '부모 클래스의 생성자를 호출' 하기 때문입니다.

//어떤 객체를 생성한다고 가정할 때
A a = new B();  > 생성자 A와 B();가 다름
같은 형태의 객체 생성 방식이 튀어나오게 되는데(추후 수업 예정)
이상의 방식은 정보처리기사 에서는 단골 문제 입니다.
 */
...
public class Product extends Item {
    ...
    //자동완성 아닌 고유 메서드 하나의 임의로 정의하겠습니다.
