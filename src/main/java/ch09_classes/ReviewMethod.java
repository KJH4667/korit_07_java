...
/*
method - 순차적으로 실행되는 여러 코드들을 하나의 이름으로 묶은것
저희는 call1() - call4()로 매개변수 유무 / return 유무로 구분했습니다.

형식: (접근지정자) 리턴타입 메서드명(매개변수1, 매개변수2) {
매서드 내부에서 순차적으로 실행될 코드를 -> 이 묶음을 비지니스 로직이라고 하기도 합니다.
}
예시:public void writeSchedule(String date, String content) {
...
public class ReviewMethod {
    // 메서드 정의 영역
    public void writeSchedule1(String date, String content) {
        System.out.println(date + "일의 스케줄은 다음과 같습니다");
        System.out.println(content);
    }

    public void writeSchedule2(String date, String content) {
        System.out.println(date + "일의 스케줄은 다음과 같습니다");
        System.out.println(content);
    }
    // 1. writeSchedule을 overloading하여 매개변수1의 자료형을 int로 바꾸고,
    // writeScedule(20250805, "클래스/getter/setter 예습"); 을 호출하시오.

    //2. call2() 유형으로 작성할겁니다.
    //dividedByseven 메서드를 정의할겁니다.
    // 특정 int 숫자 하나를 매개변수로 받아, 그 숫자가 7의 배수인지 아닌지를 확인하는 메서드를 작성하시오.
    // 실행예: 임의의 숫자를 입력하세요 >>> 77
    // 77은 7의 배수 입니다.
    //임의의 숫자를 입력하세요 >>> 100
    //100은 7의 배수가 아닙니다.
    ...
    public void dividedBySeven(int num) {
        ...
        String answer = "";
        if(num % 7 == 0) {
            answer = "7의 배수 입니다"; // 아마 대부분 여기에 sout 쓰셨을 것 같아서 이렇게 풀이합니다
        } else {
            answer = "7의 배수가 아닙니다.";
        }
